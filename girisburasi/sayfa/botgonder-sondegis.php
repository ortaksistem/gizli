...
if($islem == "gonder"){
	...
	if($email == 2){
		...
		$gondereneposta = 'bilgi@yatakpartner.com'; // from (Gmail mail address)
	...
	function mail_gonder($mail, $konu, $mesaj){
	 $kod = "123456";
	 $ch = curl_init();
	 curl_setopt($ch, CURLOPT_URL, 'http://www.mahirix.net/mail/mailal.php');
	...
	for($i; $i < $son; $i++){
		...
		*/
		...
		if($email == 3){
			...
			$mailmesaj = "Sayin uyemiz $siteadi Sitesinden $botadi rumuzlu uyemizden yeni bir mesaj aldiniz.
			Mesaji Okumak icin Lutfen Siteye Giris Yapiniz..
			
			Giris Bilgileriniz
			Kullanici Adiniz : [KID]
			Sifreniz : [KPASS]
			
			".$siteurl."index.php?sayfa=hizligiris&k=[KID]&s=[KPASS]
