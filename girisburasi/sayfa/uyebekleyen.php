...
if($islem == "sil"){
	...
	switch($tur){
		case "1":
			$tur1 = 2;
			$bekleyenler = "Admin Onayý";
		break;
		case "2":
			$tur1 = 3;
			$bekleyenler = "Email Onay";
		break;
...
function sayfa(sayfa){
	...
	window.location = 'index.php?sayfa=uyebekleyen&tur='+tur+'&p='+sayfa;
}
...
function uyesil(uye){
	...
	if(onayla){
		jQuery.ajax({
			type : 'POST',
			url : 'index.php?sayfa=uyebekleyen&islem=sil',
			data : "id="+uye,
...
function onayla(uye){
	if(onayla){
		jQuery.ajax({
			type : 'POST',
			url : 'index.php?sayfa=uyebekleyen&islem=onayla',
			data : "id="+uye,
			success: function(sonuc){	
				if(sonuc == "ok"){
					$("#uyesonuc"+uye+"").hide("slow");
