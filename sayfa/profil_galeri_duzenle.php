...
if($_SESSION["galerieklendi"] == "evet"){
	...
	$_SESSION["galeriid"] = NULL;
	
	$_SESSION["galerieklendi"] = NULL;

}

$galeribaslik = "Galeri Düzenle";
	
$galeri = $_GET["id"];

list($seninmi) = mysql_fetch_row(mysql_query("select count(id) from "._MX."galeri where id='$galeri' and uye='$uyeid'"));

if($seninmi < 1){
	?>
	<script type="text/javascript">
		alert("Sizin olmayan bir galeriyi görüntüleyemezsiniz");
		window.location = 'index.php';
	</script>
	<?
}

$_SESSION["galeriid"] = $galeri;

list($galerithumb) = mysql_fetch_row(mysql_query("select resim from "._MX."galeri where id='$galeri'"));

if(!$galerithumb){
	
	list($galerithumb) = mysql_fetch_row(mysql_query("select resim from "._MX."galeri_resim where galeri='$galeri' limit 1"));
	
	include("fonksiyon2.php");
		
	$uzanti = explode(".", $galerithumb);
		
	$uzanti = $uzanti[1];
		

		
	resmikes($galerithumb, "img_uye/galerithumb/$galeri.$uzanti");
		
	mysql_query("update "._MX."galeri set resim='img_uye/galerithumb/$galeri.$uzanti' where id='$galeri'");
...
<title><?=$galeribaslik?> ID:<?=$galeri?> <?=$uyeadi?>, <?=_BASLIK?></title>
...
<script type="text/javascript">
	...
	function resimyuklesonuc(sonuc){
		if(sonuc == "ok"){
			...
			mahirixpencereguncelle("<p align=center><font color=red><b>Yüklenemiyor, lütfen resmi kontro ediniz.</b></p>");
		}
	}
	...
	function resimguncelle(){
				...
				jQuery.ajax({
					type : 'POST',
					url : 'inc/galeri.php',
					data : "islem=resimguncelle&galeri=<?=$galeri?>",
					success: function(sonuc){		
						$("#resimguncelle").html(sonuc);
					}
				})
...
<form action="inc/galeri.php" method="post" name="resimformu" target="resimyukle" enctype="multipart/form-data">
														...
														<table border="0" id="table452" cellspacing="0" cellpadding="0">
															...
															<?
																...
																if($durum != 1) $onay = "Onay bekleniyor";
																else $onay = NULL;
															...
															?>
																...
																<table id="resimtablo<?=$resid?>" border="0" style="border-collapse: collapse" width="100%" cellpadding="0">
																	...
																	<tr>
																		<td height="6" align="center"><font color="red" size="1"><?=$onay?></font>
																		</td>
																	</tr>
																	<tr>
																		<td align="center">
